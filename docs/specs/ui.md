...
| Palette header | Keep the header simple: `Symbol palette`, the existing symbol count bubble, and a clickable libraries summary line that opens the Add/Remove Symbols dialog. Do not add an eyebrow label such as `Editor surface`, and do not introduce extra family chips below the title. |
...
| Action grouping | Present actions in this order: Branding, Diagram State (lock with state label), Insert, Tool Mode, History, View, Destructive (delete/import), Output, Help. Functional scope stays unchanged; only grouping and emphasis change. Lock icon sits immediately left of "Locked"/"Unlocked" text label. |
